...
/* Replace pthreads with simple DOS alternatives */
/* For DOS, we'll use a simpler synchronization method */
#define MUTEX_LOCK()   /* no-op in single-threaded DOS environment */
#define MUTEX_UNLOCK() /* no-op in single-threaded DOS environment */
...
void playMidiFile() {
    ...
    /* No need to call init_terminal here as it should be done in main() */
    /* Let's just make sure keep_running is set to 1 */
    keep_running = 1;
    ...
    /* No need to call restore_terminal() here as it will be done in main() */
