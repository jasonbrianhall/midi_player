...
// Time tracking for DOS (replacement for SDL_GetTicks)
...
// Get system ticks (milliseconds) for DOS
extern "C" uint32_t getTickCount(void) {
    if (start_time == 0) {
        start_time = clock();
    }
    
    // Convert CLOCKS_PER_SEC to milliseconds
    return (uint32_t)((clock() - start_time) * 1000 / CLOCKS_PER_SEC);
}
